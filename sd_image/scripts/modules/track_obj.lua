...
local gcs_send = require("gcs_send_funcfactory")("TRO")
...
local function TrackSpotClass()
    ...
    local function new(n_arg, e_arg, theta_arg)
        return setmetatable({n_arg, e_arg, theta_arg}, cls)
    end
...
local function TrackArcClass()
    ...
    local function new(s_arg, k_arg)
        return setmetatable({
            s_arg, k_arg,
            p0 = TrackSpot(0, 0, 0),
            s0_scaled = 0,
    ...
    function cls.along_arc(self, s)
        local p = along_arc_from_origin((s - self.s0_scaled) / (self.size_scale * self.s_scale), self[IDX_K])
...
local function TrackClass()
    ...
    function cls.dump(self)
        for i, arc in ipairs(self) do
            gcs_send(string.format("i: %i, %.2f, %.2f, %.2f", i, 
            arc:arc_end_spot():n(), arc:arc_end_spot():e(), arc:arc_end_spot():theta()))
        end
    end
